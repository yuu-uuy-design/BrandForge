...
const STEPS = ["intro", "form", "generating", "result"];

const INDUSTRIES = [
  "Technology", "Fashion & Apparel", "Food & Beverage", "Health & Wellness",
  "Finance", "Education", "Real Estate", "Creative Agency", "E-commerce",
  "Consulting", "Beauty & Cosmetics", "Sports & Fitness", "Travel", "Other"
];

const VIBES = [
  { id: "luxury", label: "Luxury & Premium", icon: "◆" },
  { id: "bold", label: "Bold & Disruptive", icon: "▲" },
  { id: "minimal", label: "Clean & Minimal", icon: "○" },
  { id: "playful", label: "Playful & Friendly", icon: "★" },
  { id: "tech", label: "Tech & Futuristic", icon: "⬡" },
  { id: "organic", label: "Natural & Organic", icon: "❋" },
];

const MARKETS = [
  { id: "global", label: "Global" },
  { id: "mena", label: "MENA / Arabic" },
  { id: "europe", label: "Europe" },
  { id: "us", label: "North America" },
];

// ── Palette generator from a seed color ──────────────────────────────────────
...
export default function BrandForge() {
  ...
  const loadMessages = [
    ...
    "Designing your color system…",
    "Building your voice guide…",
    "Writing your brand manifesto…",
    "Assembling your identity kit…",
    "Final touches…",
  ];
  ...
  async function generateBrand() {
    if (!form.name || !form.industry || !form.vibe) {
      setError("Please fill in brand name, industry, and vibe.");
      return;
    }
    setError("");
    setStep("generating");
    ...
    const prompt = `You are a world-class brand strategist and creative director. Generate a complete brand identity for the following business.

BUSINESS DETAILS:
- Brand Name: ${form.name}
- Industry: ${form.industry}
- Desired Vibe/Personality: ${form.vibe}
- Target Market: ${form.market}
- Additional Context: ${form.description || "None provided"}

Generate a complete brand identity package. Respond ONLY with a valid JSON object, no markdown, no preamble, no backticks. Use this exact structure:

{
  "brandName": "${form.name}",
  "tagline": "A compelling, memorable tagline (max 8 words)",
  "positioning": "One clear sentence defining the brand's unique market position",
  "targetAudience": {
    "primary": "Primary audience description",
    "psychographics": "Key values and motivations of the audience",
    "painPoints": "The main problem this brand solves for them"
  },
  "personality": {
    "archetype": "Brand archetype (e.g. The Creator, The Hero, The Sage)",
    "traits": ["trait1", "trait2", "trait3", "trait4"],
    "notTraits": ["anti-trait1", "anti-trait2"]
  },
  "voiceGuide": {
    "tone": "Overall tone description",
    "principles": ["Writing principle 1", "Writing principle 2", "Writing principle 3"],
    "doExamples": ["Example of good copy 1", "Example of good copy 2"],
    "dontExamples": ["Example of bad copy 1", "Example of bad copy 2"]
  },
  "colorSystem": {
    "primary": { "hex": "#xxxxxx", "name": "Color name", "meaning": "Why this color" },
    "secondary": { "hex": "#xxxxxx", "name": "Color name", "meaning": "Why this color" },
    "accent": { "hex": "#xxxxxx", "name": "Color name", "meaning": "Why this color" },
    "background": { "hex": "#xxxxxx", "name": "Color name", "meaning": "Usage" },
    "text": { "hex": "#xxxxxx", "name": "Color name", "meaning": "Usage" }
  },
  "typography": {
    "display": { "font": "Google Font name", "weight": "300 or 400 or 700", "usage": "Where to use" },
    "body": { "font": "Google Font name", "weight": "400", "usage": "Where to use" },
    "mono": { "font": "Google Font name or JetBrains Mono", "usage": "Where to use" }
  },
  "logoDirection": {
    "concept": "The core visual concept for the logo",
    "style": "Description of logo style (wordmark/lettermark/abstract)",
    "symbolIdea": "What the symbol/mark represents conceptually",
    "avoidances": "What visual directions to avoid"
  },
  "manifesto": "A powerful 3-4 sentence brand manifesto written in the brand voice. Make it emotionally resonant.",
  "competitors": ["Competitor 1 name", "Competitor 2 name", "Competitor 3 name"],
  "differentiator": "In one sentence, what makes this brand genuinely different from all competitors",
  "socialBio": "A ready-to-use social media bio (max 150 characters)",
  "elevatorPitch": "A 2-sentence elevator pitch for this brand"
}`;
  ...
  const css = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .intro-title {
      ...
      font-weight: 300; line-height: 0.9; margin-bottom: 32px;
    }
    ...
    .form-title {
      font-family: 'Cormorant Garamond', serif;
      font-size: clamp(28px, 5vw, 44px); font-weight: 300; margin-bottom: 12px;
    }
    .form-subtitle { font-size: 14px; color: var(--muted); }
    .form-card {
      width: 100%; max-width: 640px;
      background: var(--deep); border: 1px solid var(--border); padding: 48px 40px;
    }
    ...
    .field-input {
      ...
      color: var(--white); font-family: 'Syne', sans-serif; font-size: 15px;
    ...
    .vibe-btn {
      ...
      padding: 14px 10px; cursor: pointer; transition: all 0.25s;
    ...
    .market-btn {
      ...
      font-size: 11px; letter-spacing: 0.1em; padding: 10px 20px;
    ...
    .btn-outline {
      ...
      padding: 14px 32px; border: 1px solid var(--border); cursor: pointer;
      transition: all 0.3s;
    }
    ...
    .gen-ring {
      ...
      border: 1px solid var(--border); border-top-color: var(--gold);
      animation: spin 1.2s linear infinite; margin-bottom: 40px;
    }
    @keyframes spin { to { transform: rotate(360deg); } }
    .gen-title {
      font-family: 'Cormorant Garamond', serif;
      font-size: 32px; font-weight: 300; margin-bottom: 16px;
    }
    .gen-msg {
      font-family: 'JetBrains Mono', monospace;
    ...
    .result-header {
      text-align: center; margin-bottom: 52px; padding-bottom: 40px;
      border-bottom: 1px solid var(--border);
    }
    .result-brandname {
      font-family: 'Cormorant Garamond', serif;
      font-size: clamp(44px, 8vw, 80px); font-weight: 300; line-height: 1;
      margin-bottom: 16px;
    }
    .result-tagline {
    ...
    .result-positioning {
      font-size: 14px; color: var(--muted); max-width: 560px; margin: 0 auto 28px; line-height: 1.7;
    }
    .result-meta {
    ...
    .tab-btn {
      font-family: 'JetBrains Mono', monospace; font-size: 10px;
      letter-spacing: 0.15em; text-transform: uppercase;
      padding: 12px 22px; background: var(--surface); border: none;
      color: var(--muted); cursor: pointer; transition: all 0.25s; white-space: nowrap;
    }
    ...
    .list-item {
      font-size: 14px; color: var(--muted); display: flex;
    ...
    /* TYPOGRAPHY */
    .font-preview {
      background: var(--surface); padding: 24px 28px; margin-bottom: 12px;
    ...
    .manifesto-text span { color: var(--gold-light); }
    ...
    .copy-btn {
      ...
      background: none; border: 1px solid var(--border); padding: 7px 16px;
      cursor: pointer; transition: all 0.25s;
    }
    ...
    @media (max-width: 600px) {
      ...
      .result-actions .btn-gold { width: auto; }
    }
  `;
